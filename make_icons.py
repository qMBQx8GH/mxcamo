...
for unit_id in data:
    ...
    if source_image and os.path.isfile(source_image):
        ...
        if unit['expFactor'] != 1.0:
            icon, text = icon_text(unit['expFactor'], 'expCoeff', step)
            background = draw(background, x, y, width, step, icon, text)
            (x, y) = next_step(x, y, width, step, y_max)
        if unit['freeExpFactor'] != 1.0:
            icon, text = icon_text(unit['freeExpFactor'], 'freexpCoeff', step)
            background = draw(background, x, y, width, step, icon, text)
            (x, y) = next_step(x, y, width, step, y_max)
        if unit['crewExpFactor'] != 1.0:
            icon, text = icon_text(unit['crewExpFactor'], 'crewPointsCoeff', step)
            background = draw(background, x, y, width, step, icon, text)
            (x, y) = next_step(x, y, width, step, y_max)
        if unit['creditsFactor'] != 1.0:
            icon, text = icon_text(unit['creditsFactor'], 'creditsCoeff', step)
            background = draw(background, x, y, width, step, icon, text)
            (x, y) = next_step(x, y, width, step, y_max)
        if unit['afterBattleRepair'] != 1.0:
            icon, text = icon_text(unit['afterBattleRepair'], 'repairCoeff', step)
            background = draw(background, x, y, width, step, icon, text)
            (x, y) = next_step(x, y, width, step, y_max)
